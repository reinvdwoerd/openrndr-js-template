...
suspend fun filmgraintest() = applicationAsync {
    program {
        ...
        extend {
            grain.time = seconds * 1000
            grain.grainStrength = mouse.position.x.map(0.0, drawer.bounds.x, 0.0,0.1)
            grain.grainLiftRatio = mouse.position.y.map(0.0, drawer.bounds.y, 0.0,0.001)
